# -*- coding: utf-8 -*-
"""
This script was used to generate the propensities and parameter tables
in the supplementary text.

Created on Mon Mar 27 23:53:25 2023
...
print("Species:")
for s in cell.species.keys():
    print(f"$${s}$$: ")

print("\nReaction propensities:")
for p in cell.reactions:
    species = list(p.stoichiometry.keys())[0]
    stoi = p.stoichiometry[species]
    if stoi > 0:
        reaction = f"0 -> {species}\t"
    else:
        reaction = f"{species} -> 0\t"
    
    print(reaction+"$$"+py2tex(p.propensity)+"$$")

print("\nParameter values:")
for k in cell.params.keys():
    print(f"$${k}$$\t", end="")
print("")

for v in cell.params.values():
    print(f"{v}\t", end="")