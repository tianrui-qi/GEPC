...
class ODEModel():
    ...
    def __init__(self):
        ...
        self._bounds = {
            'c2':[0, np.inf],
            'a':[0, np.inf], 
            'nu':[.0058/self._init_params["nu"], .0462/self._init_params["nu"]], # Cell division period between 15min and 2 hours
    ...
    def ccasr_ode(t, y, u, params):
        ...
        # First order moments
        L = (H**params["nh"])/(params["K_H"]**params["nh"]+H**params["nh"])
        dH_dt = params["eta"]*u - params["nu"]*H
        dE_dt = params["h1"] - params["h2"]*E
        dF_dt = params["a"]*E*L - params["nu"]*F
        ...
        if len(y)==6: # Also compute second order moments
            ...
            dEF_dt = params["h1"]*F + params["a"]*L*E2 - (params["h2"] + params["nu"])*EF
            dF2_dt = params["nu"]*F + params["a"]*L*E + 2*params["a"]*L*EF - 2*params["nu"]*F2
...
#%%
import os
mpath = "Y:/projectnb/dunlop/hklumpe/deepcellcontrol/assets/models/"
models = [m for m in os.listdir(mpath) if m.startswith("2023")]

all_params = []
for m in models:
    print(m, end=": ")
    with open(mpath + m + "/training_parameters.json", "r") as f:
        params = json.load(f)
    all_params.append(params)
    print(params["past_steps"])

#%%

for m, params in zip(models, all_params):
    
    if params["past_steps"] == 36 and params["cell_class"] == "CcaSR_gillespie":
        print(m, end=": ")
        print(params["training_sets"][0])
