...
# Patch batch methods:
import types
def nostimsbatch(self):
    X, Y = dcc.data.Datasets.batch(self)
    X[0] = X[0][:,:,self.features.index("fluo1"), np.newaxis]
    return X, Y
training_set.batch = types.MethodType(nostimsbatch, training_set)
evaluation_set.batch = types.MethodType(nostimsbatch, evaluation_set)

# Init model:
newparams = copy.deepcopy(params)
newparams["features"] = ("fluo1",)
